...
use crate::{
  ...
  whisper::{GenerationConfig, LiteWhisperPipeline, WhichLiteWhisperModel},
};
...
struct Args {
  ...
  #[arg(long, default_value = "large-v3-turbo-fast")]
  whisper_model:           Option<WhichLiteWhisperModel>,
}
...
async fn main() -> Result<()> {
  ...
  let mut whisper = LiteWhisperPipeline::new(model_name, revision)?;
...
async fn process_audio_chunk(
  ...
  whisper: &mut LiteWhisperPipeline,
