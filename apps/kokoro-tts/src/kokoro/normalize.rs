// https://github.com/lucasjinreal/Kokoros/blob/4c18036a174d28ddec3181c99395af332c32b28d/kokoros/src/tts/normalize.rs
...
lazy_static! {
  ...
  static ref NUMBERS_RE: Regex = Regex::new(r"\d*\.\d+|\b\d{4}s?\b|(?<!:)\b(?:[1-9]|1[0-2]):[0-5]\d\b(?!:)").unwrap();
  static ref COMMA_NUM_RE: Regex = Regex::new(r"(?<=\d),(?=\d)").unwrap();
  static ref MONEY_RE: Regex = Regex::new(r"(?i)[$£]\d+(?:\.\d+)?(?: hundred| thousand| (?:[bm]|tr)illion)*\b|[$£]\d+\.\d\d?\b").unwrap();
