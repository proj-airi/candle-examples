...
impl Filter {
  ...
  pub fn apply(&mut self, x0: f32) -> f32 {
    let y0 = 0.0 + self.b0 * x0 + self.b1 * self.x1 + self.b2 * self.x2 - self.a1 * self.y1 - self.a2 * self.y2;
...
impl<T> Windows100ms<T> {
  ...
  where
    T: AsMut<[Power]>,
  {
    Windows100ms { inner: self.inner.as_mut() }
  }
...
pub fn reduce_stereo(left: Windows100ms<&[Power]>, right: Windows100ms<&[Power]>) -> Windows100ms<Vec<Power>> {
